...
useEffect(() => {
  ...
  //setIsLoading(true);
          ...
          //const userEntries; 
  const q = query(collection(db, "PastorRandy", "Entries", "entryData"), where("type", "==", "Audio" ),  orderBy("creationDate", "desc"), limit(10));
  ...
  const loadMoreMusic_Video = async () => {
    ...
    //const userEntries; 
  const q = query(collection(db, "PastorRandy", "Entries", "entryData"), where("type", "==", "Audio" ),  orderBy("creationDate", "desc"), startAfter(lastSeen), limit(10));
...
const renderItem = ({ item }) => (   <Box h="150px" w="100%" mr="5" mb="5"  borderRadius={"md"} borderWidth="1" borderColor={"coolGray.500"} >
...
<FlatList mt="4" h="65%" data={data} extraData={updatedData} ListEmptyComponent={entryEmpty} renderItem={renderItem} keyExtractor={item => item.title}  ListFooterComponent={entryFooter}  onEndReached={loadMoreMusic_Video} onEndReachedThreshold={0.5}
