...
useEffect(() => {
  ...
  //setIsLoading(true);
          ...
          //const userEntries; 
  const q = query(collection(db, "PastorRandy", "Entries", "entryData"), where("type", "==", "Video" ), orderBy("creationDate", "desc"), limit(10));
  ...
  const loadMoreSermons = async () => {
    ...
    //const userEntries; 
  const q = query(collection(db, "PastorRandy", "Entries", "entryData"), where("type", "==", "Video" ),  orderBy("creationDate", "desc"), startAfter(lastSeen), limit(10));
...
<FlatList mt="4" h="65%" data={data} extraData={updatedData} ListEmptyComponent={entryEmpty} renderItem={renderItem} keyExtractor={item => item.title}  ListFooterComponent={entryFooter}  onEndReached={loadMoreSermons} onEndReachedThreshold={0.5}
