...
useEffect(() => {
  ...
  //setIsLoading(true);
          ...
          //const userEntries; 
  const q = query(collection(db, "PastorRandy", "Entries", "entryData"), where("type", "==", "Movies" ), orderBy("creationDate", "desc"), limit(10));
  ...
  const loadMoreMovies = async () => {
    ...
    //const userEntries; 
  const q = query(collection(db, "PastorRandy", "Entries", "entryData"), where("type", "==", "Movies" ), orderBy("creationDate", "desc"), startAfter(lastSeen), limit(10));
...
<FlatList mt="4" h="65%" data={data} extraData={updatedData} ListEmptyComponent={entryEmpty} renderItem={renderItem} keyExtractor={item => item.title}  ListFooterComponent={entryFooter}  onEndReached={loadMoreMovies} onEndReachedThreshold={0.5}
